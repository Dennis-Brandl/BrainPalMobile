...
import type { RunnerConfig, RecoveryResult, WorkflowRunnerState } from './types';
...
export async function recoverWorkflows(
  ...
  for (const workflow of activeWorkflows) {
    try {
      ...
      const stepsToReactivate: string[] = [];
      ...
      for (const step of steps) {
        ...
        switch (recoveryAction) {
          ...
          case 'reactivate':
            ...
            stepsToReactivate.push(step.step_oid);
            break;
          ...
          case 'stay-executing':
            // USER_INTERACTION/YES_NO in EXECUTING -- re-display form
      ...
      result.recovered.push(workflow.instance_id);
...
function getRecoveryAction(step: RuntimeWorkflowStep): RecoveryAction {
  switch (step.step_state) {
    ...
    case 'EXECUTING':
      // User interaction steps stay in EXECUTING (re-display form)
